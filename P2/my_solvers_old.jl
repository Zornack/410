...
"""
    pivot!(A,j)
Partially pivots matrix A, putting the largest value below 
and including A[j,j] into pivot position. Also stores the pivot 
operation applied to A in order to pivot the LTM. 
"""
function pivot!(A,j)
    M = size(A,1)
    N = size(A,2)
...
and then solving L*ƀ=y and U*y = x. Returns the solution as vector x. 
"""
...
"""
    testMatrix(N)
Creates an N by N text matrix A and a random vector b for
...
and stores the matrix sizes in xSolves and the time to solve in ySolves.
"""
